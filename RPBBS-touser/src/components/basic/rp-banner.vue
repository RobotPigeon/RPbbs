...
<div class="relative">
    <div class="overflow-hidden">
      <div class="flex transition-all duration-500 ease-in-out" :style="{ transform: `translateX(-${currentSlide * slideWidth}%)` }">
        <div v-for="(slide, index) in props.list" :key="index" class="w-full flex-shrink-0 overflow-hidden xl:h-70 md:h-60 sm:h-50 cellphone bg-black relative">
        <img :src="slide" alt="" class="xl:h-70 md:h-60 sm:h-50 cellphone w-full  m-a">
        </div>
    </div>
    </div>
