            ...
            <div class="flex w-full mx-auto px-0 py-0 justify-center">
                <div class="stats shadow">
                    ...
                    <div class="stat">
                        ...
                        <div class="stat-title">比赛次数</div>
                        <div class="stat-value text-secondary">{{ playerInfo.mp }}</div>
                        <div class="stat-title">参赛时间</div>
                    ...
                    <div class="stat">
                        ...
                        <div class="stat-value">人数:{{ playerInfo.numPl }}人</div>
                        <div class="stat-title">代表队:{{ playerInfo.squad }}</div>
                        <div class="stat-desc text-secondary">平均年龄:{{ playerInfo.avgAge }}</div>
                    </div>
                ...
                <div>
                    <div class="stats stats-vertical shadow mt10">
                        <div class="stat">
                            ...
                            <div class="stat-value">{{ teamShooting.sh }}次</div>
                            <div class="stat-desc">{{ teamShooting.sh90 }}(每90分钟)</div>
                            <div class="stat-desc">平均射门距离:{{ teamShooting.adist }}m</div>
                        </div>

                        <div class="stat">
                            <div class="stat-title">射正次数</div>
                            <div class="stat-value">{{ teamShooting.soT }}次</div>
                            <div class="stat-desc">{{ teamShooting.sot90 }}(每90分钟)</div>
                        </div>
                    </div>
                </div>
                <div>
                    <div class="stats stats-vertical shadow mt10">
                        <div class="stat">
                            ...
                            <div class="stat-value">{{ teamShooting.xg }}</div>
                            <div class="stat-desc">{{ teamShooting.npxG }}(去除点球)</div>
                        </div>

                        <div class="stat">
                            <div class="stat-title">非运动战射门(点球)</div>
                            <div class="stat-value">{{ teamShooting.pkatt }}次</div>
                            <div class="stat-desc">任意球：{{ teamShooting.fk }}次</div>
                            <div class="stat-desc">点球命中：{{ teamShooting.pk }}次</div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div>
                    <div class="stats stats-vertical shadow mt10">
                        <div class="stat">
                            ...
                            <div class="stat-value">{{ teamDefense.tkl }}次</div>
                            <div class="stat-desc">前场{{ teamDefense.att3rd }}次</div>
                            <div class="stat-desc">中场{{ teamDefense.mid3rd }}次</div>
                            <div class="stat-desc">后场{{ teamDefense.def3rd }}次</div>
                        </div>
                        <div class="stat">
                            <div class="stat-title">拦截表现</div>
                            <div class="stat-value">{{ teamDefense.blocks }}次</div>
                            <div class="stat-desc">射门拦截{{ teamDefense.shotBlock }}次</div>
                            <div class="stat-desc">传球拦截{{ teamDefense.passBlock }}次</div>
                        </div>
                    </div>
                </div>
                <div class="flex flex-col">
                ...
                <div>
                    <div class="stats stats-vertical shadow mt10">
                        <div class="stat">
                            ...
                            <div class="stat-value">{{ teamDefense.catt }}</div>
                            <div class="stat-desc">成功{{ teamDefense.ctkl }}次</div>
                            <div class="stat-desc">失败{{ teamDefense.ctklL }}次</div>
                        </div>
                        <div class="stat">
                            <div class="stat-title">解围</div>
                            <div class="stat-value">{{ teamDefense.clear }}</div>
                            <div class="stat-desc">抢断{{ teamDefense.tkl }}次</div>
                            <div class="stat-desc">抢断成功{{ teamDefense.tklW }}次</div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div>
                    <div class="stats stats-vertical shadow mt10">
                        <div class="stat">
                            ...
                            <div class="stat-value">{{ teamPassing.totalAtt }}次</div>
                            <div class="stat-desc">长传{{ teamPassing.longAtt }}次</div>
                            <div class="stat-desc">中传{{ teamPassing.mediumAtt }}次</div>
                            <div class="stat-desc">短传{{ teamPassing.shortAtt }}次</div>
                        </div>
                        <div class="stat">
                            <div class="stat-title">传球成功次数</div>
                            <div class="stat-value">{{ teamPassing.totalCmp }}次</div>
                            <div class="stat-desc">长传{{ teamPassing.longCmp }}次</div>
                            <div class="stat-desc">中传{{ teamPassing.mediumCmp }}次</div>
                            <div class="stat-desc">短传{{ teamPassing.shortCmp }}次</div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <div class="stats stats-vertical shadow mt10">
                        <div class="stat">
                            ...
                            <div class="stat-value">{{ teamPassing.kp}}</div>
                            <div class="stat-desc">进攻三区直传球:{{  teamPassing.lastThird}}次</div>
                            <div class="stat-desc">成功传中:{{  teamPassing.crsPA}}次</div>
                        </div>

                        <div class="stat">
                            <div class="stat-title">传球距离</div>
                            <div class="stat-title">传球总距离</div>
                            <div class="stat-value">{{ teamPassing.totalTotDist }}m</div>
                            <div class="stat-title">向前传球距离</div>
                            <div class="stat-value">{{ teamPassing.totalPrgDist }}m</div>
                        </div>
                    </div>
                </div>
            </div>
...
//引入api获取球员信息
import { getTeamById } from '@/api/football';
...
const teamShooting: Ref<any> = ref({});
const teamDefense: Ref<any> = ref({});
const teamPassing: Ref<any> = ref({});
...
function getPlayerInfo() {
    getTeamById(id).then((res: any) => {
        playerInfo.value = res.data;
        teamShooting.value = res.data.teamShooting;
        teamDefense.value = res.data.teamDefense;
        teamPassing.value = res.data.teamPassing;
        console.log(playerInfo.value);
        console.log(teamPassing.value);
        //取出球员信息
        //导入射门射正次数和进球次数和射失次数
        optionESAG.series[0].data[0].value = res.data.teamShooting.soT;
        optionESAG.series[0].data[1].value = res.data.teamShooting.sh - res.data.teamShooting.soT;
        //导入三区防守数据
        optionED3RD.series[0].data[0].value = res.data.teamDefense.att3rd;
        optionED3RD.series[0].data[1].value = res.data.teamDefense.mid3rd;
        optionED3RD.series[0].data[2].value = res.data.teamDefense.def3rd;
        //导入封堵数据
        optionEBlock.series[0].data[0].value = res.data.teamDefense.shotBlock;
        optionEBlock.series[0].data[1].value = res.data.teamDefense.passBlock;
        //导入对抗数据
        optionECtkl.series[0].data[0].value = res.data.teamDefense.ctkl;
        optionECtkl.series[0].data[1].value = res.data.teamDefense.ctklL
        //导入传球数据
        optionEpass.series[0].data[0] = res.data.teamPassing.longAtt;
        optionEpass.series[0].data[1] = res.data.teamPassing.mediumAtt;
        optionEpass.series[0].data[2] = res.data.teamPassing.shortAtt;
        optionEpass.series[1].data[0] = res.data.teamPassing.longAtt-res.data.teamPassing.longCmp;
        optionEpass.series[1].data[1] = res.data.teamPassing.mediumAtt-res.data.teamPassing.mediumCmp;
        optionEpass.series[1].data[2] = res.data.teamPassing.shortAtt-res.data.teamPassing.shortCmp;
        //导入传球距离数据
        optionEPassD.series[0].data[0].value = res.data.teamPassing.totalPrgDist;
        optionEPassD.series[0].data[1].value = res.data.teamPassing.totalTotDist - res.data.teamPassing.totalPrgDist;
        charshootandgoal();
        chardefense();
        charblock();
        charctkl();
